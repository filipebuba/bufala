...
class SmartApiService {
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) {
          ...
          print('[📋 SMART-API] Dados: ${options.data}');
        ...
        onError: (error, handler) {
          print('[❌ SMART-API] Erro: ${error.message}');
          handler.next(error);
        },
      ),
    );
  ...
  Future<SmartApiResponse<String>> askMedicalQuestion({
    ...
    try {
      ...
      print('[🏥 SMART-API] Pergunta: ${question.substring(0, math.min(100, question.length))}...');
      ...
      final response = await tempDio.post<Map<String, dynamic>>(
        ...
        data: {
          ...
          'timeout_preference': 'fast', // Indicar preferência por resposta rápida
        },
      );
      ...
      // Se for timeout, tentar fallback médico
      if (e.type == DioExceptionType.receiveTimeout ||
          e.type == DioExceptionType.connectionTimeout) {
        print('[🔄 SMART-API] Timeout detectado, usando fallback médico...');
        return _getMedicalFallback(question, language);
      }
  ...
  SmartApiResponse<String> _getOfflineMedicalResponse(
    ...
    if (questionLower.contains('febre') || questionLower.contains('temperatura')) {
      response = '''
🌡️ **Sobre Febre:**
...
''';
    } else if (questionLower.contains('dor de cabeça') || questionLower.contains('cefaleia')) {
      response = '''
🧠 **Sobre Dor de Cabeça:**
...
''';
    } else if (questionLower.contains('pressão') || questionLower.contains('hipertensão')) {
      response = '''
